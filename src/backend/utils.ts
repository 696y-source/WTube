...
export function getVideoId(song: YTNodes.MusicResponsiveListItem): string {
  return song.id ||
    ...
    song.flex_columns?.find(
      c =>
        c.is(YTNodes.MusicResponsiveListItemFlexColumn) &&
        c.as(YTNodes.MusicResponsiveListItemFlexColumn).title?.endpoint?.name === 'watchEndpoint'
    )?.as(YTNodes.MusicResponsiveListItemFlexColumn).title?.endpoint?.payload?.videoId ||
    (
      song.menu?.items?.find((i: any) => i.endpoint?.payload?.videoId) as any
...
export function streamMapper(node: Helpers.YTNode): YTItem | null {
  if (node.is(YTNodes.Video)) {
    ...
    const views =
      video.short_view_count?.toString() ||
      video.view_count?.toString();
    const published =
      video.published?.toString()?.replace('Streamed ', '');
    const subtext =
      (views || '') +
      (published ? ' • ' + published : '');
    return {
...
export function listMapper(node: Helpers.YTNode): YTListItem | null {
  if (node.is(YTNodes.LockupView)) {
    ...
    if (lockup.content_id?.startsWith('RD'))
      return null;
    const metadata =
      lockup.metadata?.as(YTNodes.LockupMetadataView);
    const contentImage =
      lockup.content_image?.as(YTNodes.CollectionThumbnailView);
    const thumbUrl =
      contentImage?.primary_thumbnail?.image?.[0]?.url || "";
    ...
    if (type === 'album') {
      const pId =
        ...
        item.menu?.items?.find(
          (i: any) =>
            i.is(YTNodes.MenuNavigationItem) &&
            i.as(YTNodes.MenuNavigationItem).endpoint?.payload?.playlistId
        )?.as(YTNodes.MenuNavigationItem).endpoint?.payload?.playlistId ||
        "";
      const artistName =
        item.author?.name ||
        item.artists?.[0]?.name ||
        item.subtitle?.runs?.find(
          (r: any) => r.endpoint?.payload?.browseId?.startsWith('UC')
        )?.text ||
        "Unknown Artist";
      return {
        id: item.id || "",
